...
It also writes the status line, which has a second job now that no preset ships treasures: with `state.pieces` empty there is nothing to solve, and the normal line would read "Remaining to find: none. Exact over 1 layout", i.e. "you're done" on an untouched board. So it becomes the prompt to enter them, pointing at the setup panel with an arrow. Both arrows are in the markup and the `720px` breakpoint shows one (`←` on desktop, where the controls are the left column; `↓` on mobile, where `.stage-area { order: -1 }` puts them below the board), so the copy stays a single string and nothing has to ask `matchMedia`.
...
`STAGES` holds the 24 real stages, but **grid size and pickaxes/tile only**: every `pieces` list is empty. The game patch *"Added variety to levels. Now, a set of treasures will be selected at random from several sets of similar difficulty for the current level"* means a stage no longer has *the* treasure list, so a fixed one-per-stage preset would be wrong more often than right. Grid size and pickaxes/tile did not change, so those still autofill.

Emptying them was a data change, not a code change, because an empty `pieces` already had a defined meaning end to end: the board loads empty, `renderStageInfo()` prints `setup.treasuresVary` instead of the dimension list, and `recompute()` replaces the status line with the no-treasures notice. The one thing that had to go was `stageLabel()`'s "(no data)" marker, which existed to flag the odd unpublished stage and would now be on all 24.

`HIDDEN_STAGES` holds stages loadable by `loadStage()` but **not** rendered into the dropdown (`ALL_STAGES = STAGES.concat(HIDDEN_STAGES)`), keyed by negative `n`: `-1` is the empty board for the "nothing left to find" path, and `-101..-124` are the **pre-patch treasure lists** for stages 1..24. Those are not dead data. Each is one real set the stage can still draw, and the tests need boards that actually have treasures on them (`FIX` in the test file maps them). They stay out of the dropdown because nothing can tell a player which set they got.

Editing pieces/grid manually switches the dropdown back to "custom". Note that this now happens to *everyone* returning with a saved board: `matchesStage()` compares the save against the stage's current definition, sees treasures where the preset has none, and relabels it custom while keeping the board (there is a test for exactly this).

### Bringing presets back

The shape this wants is a list of sets per stage plus a picker for which one you got, which is why the patch notice (`#noticeDialog`, `NOTICE_V`) asks for screenshots of the stage number and its treasure list. `NOTICE_V` is a version string rather than a flag precisely so that the "presets are back" notice can re-fire for everyone by changing it.
...
[`tests/app.test.js`](../tests/app.test.js) loads `index.html` in jsdom and drives the real DOM (`node --test`, i.e. `npm test`). It covers boot, digging, the popover regression, preset loading (grid and pickaxes, no treasures), the no-treasures status line, the **patch notice** (opens once per browser, remembered by version, reopened from the setup panel, and kept shut when `localStorage` is unavailable), the custom-switch, the best-tile highlight, the dug/buried split and toggle, the empty-board path, the estimator (including the regression that it counts every treasure tile, not one hit per treasure), the **DP toggle** (exact `(DP)` engine on a dense stage where DFS bails, and the Monte-Carlo fallback when it's off), **persistence** (a board round-tripping through a refresh, restored treasure identity, the stale-preset fallback to custom, corrupt saves ignored, and New game not resurrecting the old board), **i18n** (the picker's per-locale links, a prerendered page booting in its language, region-aware auto-detection on the root, and that treasure names never surface), and the **per-locale pages** (reciprocal `hreflang`, self-canonicals, the pin outranking a stored preference, `../` asset paths, and the sitemap).
