...
Measured effect vs the conservative estimate: ~13.5% fewer pickaxes overall (per-set 2 to 25%, more on sparse boards that force more empty digs). Variance stays tight (cv ~0.1) and is not wider than the no-bomb estimate. Perf is a non-issue: a 600-trial call is ~13 ms on the densest stage, ~100× under `EST_TIME_MS`.

#### The three deviating weight tables (`BOMB_W0`)

Three of the game's 48 deviate from the standard table, and only in the **zero-bomb weight**, so `E[k]` is always `9500/total`. `BOMB_W0` maps `"stage:set"` to the override and `useBombTable()` resolves it once per estimate (`rollBomb()` runs deep inside the trial loop). `bombTotal` is read off the table rather than hardcoded to 10000, which was a latent bug waiting for exactly this change: a stale 10000 turns every roll past the end of an 8000-weight table into a 0, giving *fewer* bombs precisely where the deviation grants more.

| applies to | weight-0 | bomb fires | `E[k]` |
|---|---|---|---|
| everything else (45 of 48) | 4000 | 60.0% | 0.95 |
| stage 6, set 1 | 2000 | 75.0% | 1.19 |
| stage 7, set 1 | 2000 | 75.0% | 1.19 |
| stage 8's unrecorded set | 5000 | 54.5% | 0.86 |

**It reads as difficulty compensation, and the estimator measures it working.** At 15,000 trials per set, those two sets are the two hardest alternates in the game with bombs *off* (stage 6 set 1 is +11.7% over its stage's other set, stage 7 set 1 is +25.3%, the worst in the game). With their real bomb rate applied they fall back in line, to **+1.6%** and **+3.3%**. The compensation is sized almost exactly to the difficulty gap it offsets, which is strong evidence the mapping is right.

That leaves **one** unverified input rather than two: bomb targeting is assumed uniform over unopened cells. That, plus the third deviation having no set to attach to yet, is why the toggle still defaults off.
...
It also writes the status line, which has a second job when `state.pieces` is empty: there is nothing to solve, and the normal line would read "Remaining to find: none. Exact over 1 layout", i.e. "you're done" on a board that has not been started. So it becomes the prompt to enter them, pointing at the setup panel with an arrow. Presets ship treasures again, so this is no longer the state every stage opens in, but it is still one click away (clear the pieces, or build a custom board). Both arrows are in the markup and the `720px` breakpoint shows one (`←` on desktop, where the controls are the left column; `↓` on mobile, where `.stage-area { order: -1 }` puts them below the board), so the copy stays a single string and nothing has to ask `matchMedia`.
...
`STAGES` holds the 24 real stages: grid size, pickaxes/tile, and a **`sets` array** of the treasure lists that stage can roll. The game patch *"Added variety to stages. Now, a set of treasures will be selected at random from several sets of similar difficulty for the current stage"* means a stage has no single treasure list, so a preset is a list of lists and `loadStage(n, setIdx)` loads one of them (`sets[0]` by default, which is what the stage shipped with before the patch).

**The solver's dimensions-only view collapses most of the game's variants.** A stage can have up to three, but most of the difference between them is a reskin: the same rectangles with different treasures sitting in them. Since `state.pieces` is keyed by size, a reskin *is* the same set here and is stored once. Player screenshots show this happening directly, with stage 1 appearing both as three cola bottles and as three trumpets, the same three 1×3 either way. That is why 24 stages need only 36 sets, and why **16 stages have exactly one set** and never have to ask (the 15 from stage 10 up, plus stage 8, whose second set is unrecorded). A test asserts no stage lists the same set twice, because two identical sets would give the chooser two identical options.

**The stage table is not exhaustive.** Stage 2's third set (`1×2 (×2), 1×3 (×2)`) is not in it and came from a player screenshot; it is dimensionally distinct from the other two, so it is not a reskin the dedup swallowed. A stage can hold three variants and the table listed two for stage 2, so treat a sighting that matches no option as new data rather than a misread, on any stage and not only the two carrying `partial`.

Stages 8 and 9 carry `partial: true`: the game has one more set for each that isn't recorded. `renderStageInfo()` appends `setup.partialSets` on those two, so a player whose board matches none of the options knows it isn't their mistake.
...
- `renderSetPicker(s)` fills `#setSelect` and shows `#setRow` only when `s.sets.length > 1`. Options are labelled by dimensions (`dimsLabel(setDims(set))`, e.g. `1×2 (×2), 2×2 (×2), 2×4`) because **treasure names are never displayed**; the no-duplicates property is what makes those labels unambiguous.
  - *Gotcha, and it shipped broken once:* it hides via the `hidden` attribute, and the UA stylesheet's `[hidden] { display: none }` loses to **any** author `display` rule (author origin beats UA regardless of specificity). `#setRow` is a `.row`, which is `display: flex`, so the picker rendered as an empty box on all 16 single-set stages. `styles.css` now forces `[hidden] { display: none !important }`. No DOM test can catch this class of bug, because `boot()` strips the stylesheet before jsdom sees it, so the guard is a test that asserts on `styles.css` as text.
- `currentSet` tracks which set is loaded, and `currentStage()` reads the stage back off the dropdown.
- Editing pieces/grid by hand calls `markCustom()`, which drops the dropdown to "(custom)" **and** hides the picker: hand-edited pieces are no stage's set, so there is nothing left to pick between.

`HIDDEN_STAGES` holds stages loadable by `loadStage()` but **not** rendered into the dropdown (`ALL_STAGES = STAGES.concat(HIDDEN_STAGES)`), keyed by negative `n`. Only two remain: `-1`, the empty board for the "nothing left to find" and no-treasures paths, and `-2`, a single 1×2 for the all-dug-out path. The `-101..-124` pre-patch lists are gone, folded into each stage's `sets[0]`, so the treasure data has one home; `FIX` in the test file now points at real stages.
...
- **Stale presets, and which set you were on.** The saved `cells` only make sense against the saved `pieces`, so `pieces` are restored from storage rather than re-derived from `STAGES`. `stageSetOf(s, b)` then asks which of the stage's sets the saved board holds and returns its index, or `-1` for none: the dropdown and the set picker follow that answer, so a board saved on a stage's second set comes back on the second set. **The index is derived, never stored**, which is why the save format did not have to change and cannot drift from the pieces it describes. A save that matches no set (a hand-edited board, a set that changed underneath, or one written while the presets were empty) keeps the board and drops the label to "custom" rather than claiming a stage whose definition has moved on.
...
- **`I18N`** maps each language code to a flat `key → string` table (96 keys). **`LANGS`** is the ordered `[code, autonym]` list that fills the header `#langSelect` (languages are listed in their own name). A test asserts every locale defines the full English key set, so a locale can never half-silently fall back to English.
...
[`tests/app.test.js`](../tests/app.test.js) loads `index.html` in jsdom and drives the real DOM (`node --test`, i.e. `npm test`). It covers boot, digging, the popover regression, preset loading (grid, pickaxes, and the stage's first set), the **set picker** (shown only on multi-set stages, labelled by dimensions, hidden again on a manual edit) and the partial-stage hint, a **data check** that every one of the 36 sets has at least one valid layout on its board and that no stage lists the same set twice, the no-treasures status line, the **patch notice** (opens once per browser, remembered by version, reopened from the setup panel, and kept shut when `localStorage` is unavailable), the custom-switch, the best-tile highlight, the dug/buried split and toggle, the empty-board path, the estimator (including the regression that it counts every treasure tile, not one hit per treasure), the **DP toggle** (exact `(DP)` engine on a dense stage where DFS bails, and the Monte-Carlo fallback when it's off), **persistence** (a board round-tripping through a refresh, restored treasure identity, a board saved on a stage's second set coming back on that set, the stale-preset fallback to custom, corrupt saves ignored, and New game not resurrecting the old board), **i18n** (the picker's per-locale links, a prerendered page booting in its language, region-aware auto-detection on the root, and that treasure names never surface), and the **per-locale pages** (reciprocal `hreflang`, self-canonicals, the pin outranking a stored preference, `../` asset paths, and the sitemap).

Two wrinkles worth knowing:

- `boot()` **inlines the external scripts** into the HTML string before handing it to jsdom. `runScripts: "dangerously"` does not fetch `<script src>`, and `resources: "usable"` would make every boot async and force all 48 tests to await a load event. Inlining in place is faithful (same order; classic scripts have no other load semantics) and keeps `boot()` synchronous. The stylesheet is dropped, which changes nothing: jsdom never resolved the old inline `<style>` either, which is why the contrast test carries its own copy of the colours.
