...
A six-card carousel that teaches the whole tool, opened from a `?` button in the header (`#tutBtn`). On a suspected first visit it is **not** auto-opened; instead the button gets a one-time **nudge** (a pulse plus a small "New here?" bubble, `showNudge`). The trade-off is deliberate: a modal that opens itself is the one genuinely intrusive outcome for a returning visitor, and this app remembers boards, so returning visitors are common.

**The nudge gate (`maybeFirstRunNudge`, run at the end of boot).** It fires only when all three hold: storage is writable (`tutStorageOK`), `localStorage["th.seenTutorial"]` is absent (`tutSeen`), and `restoreBoard()` returned false (no board saved on this browser). The saved-board check is load-bearing, because the first page load auto-writes a board (`loadStage(1)` → `recompute()` → `saveBoard()`), so "no `th.board`" is only ever true at the boot fork itself; `boot()` captures `const hadSavedBoard = restoreBoard()` for exactly this. Two consequences: a browser that has ever used the tool is never interrupted (its saved board suppresses the nudge, which also spares existing users on the deploy that ships this), and when storage is unwritable the nudge is skipped rather than repeated every visit (it could never persist "seen"). The flag is set the moment the nudge shows or the tutorial opens (`markTutorialSeen`), so it appears at most once; the `?` button replays it anytime. No cookie, no server, no fingerprinting: `localStorage` is the only signal, the same one the board and language prefs already trust.

**The carousel (`TUT_CARDS`, `buildTutDialog`, `renderTut`, `openTutorial`/`closeTutorial`).** A native `<dialog>` built once and memoised, opened with `showModal()` and a fallback to the `open` attribute where unsupported (the `creditsDialog` pattern, so jsdom's unimplemented `showModal` degrades cleanly). It carries progress dots, a Skip, Back/Next, and a **Done** on the last card; Esc and a backdrop click close it, Left/Right arrows step it. It is responsive off the same `@media (max-width: 720px)` breakpoint the popover uses: a centered card on desktop, a bottom sheet on mobile.

**The card figures.** Cards 1, 3, 4, 5 draw little boards in the DOM (`tutBoard`, coloured with the real `heat()`/`inkFor()`), so they need no image assets and localise for free. Cards 2 (stage setup) and 6 (the estimate/bomb panel) instead show a cropped **screenshot of the real controls**, one per locale (`tutShot`), because those cards point at specific UI the copy names. The images are `tutorial-{setup,estimate}-<lang>.png` at the repo root; `tutShot` builds the `<img>` at runtime, so it prefixes the src with `../` on a `/<locale>/` page (`TUT_ASSET_PREFIX`, read off the same pin the generator stamps in) and falls back to the English crop via `onerror`. These screenshots are committed assets with no build step wired for them, so they go stale if the controls markup changes: regenerate them by rendering each locale page headlessly and cropping the setup and estimate sections.

**Copy and i18n.** The 19 `tutorial.*` keys follow the house rules: plain and factual (no marketing, no taglines) and no em dashes. Where a card names a control in **bold** (Preset, Empty, Place it, Estimate picks to finish, Account for bombs) it quotes that locale's actual label (`setup.preset`, `dig.empty`, `place.placeIt`, `estimate.button`, `bomb.label`), and the event/pickaxe/stage words follow the game glossary. The 15 non-English locales were drafted and then reviewed **in context**, one reviewer per locale reading that locale's screenshots and established voice; most fixes were consistency with the app's own terms (the dig verb, the treasure item, the word for "board").
...
- **`I18N`** maps each language code to a flat `key → string` table (105 keys, 19 of them the `tutorial.*` walkthrough strings). **`LANGS`** is the ordered `[code, autonym]` list that fills the header `#langSelect` (languages are listed in their own name). A test asserts every locale defines the full English key set, so a locale can never half-silently fall back to English.
...
[`tests/app.test.js`](../tests/app.test.js) loads `index.html` in jsdom and drives the real DOM (`node --test`, i.e. `npm test`). It covers boot, digging, the popover regression, preset loading, the custom-switch, the best-tile highlight, the dug/buried split and toggle, the empty-board path, the estimator (including the regression that it counts every treasure tile, not one hit per treasure), the **DP toggle** (exact `(DP)` engine on a dense stage where DFS bails, and the Monte-Carlo fallback when it's off), **persistence** (a board round-tripping through a refresh, restored treasure identity, the stale-preset fallback to custom, corrupt saves ignored, and New game not resurrecting the old board), **i18n** (the picker's per-locale links, a prerendered page booting in its language, region-aware auto-detection on the root, and that treasure names never surface), the **per-locale pages** (reciprocal `hreflang`, self-canonicals, the pin outranking a stored preference, `../` asset paths, and the sitemap), and the **first-run walkthrough** (the `?` button opening the carousel and stepping through to Done, on desktop and touch, plus the nudge gate: shown once on a fresh browser, suppressed by a saved board and by the seen flag).

Two wrinkles worth knowing:

- `boot()` **inlines the external scripts** into the HTML string before handing it to jsdom. `runScripts: "dangerously"` does not fetch `<script src>`, and `resources: "usable"` would make every boot async and force all 43 tests to await a load event. Inlining in place is faithful (same order; classic scripts have no other load semantics) and keeps `boot()` synchronous. The stylesheet is dropped, which changes nothing: jsdom never resolved the old inline `<style>` either, which is why the contrast test carries its own copy of the colours.
