...
**The third deviation is the mirror image, and it measures that way too.** Stage 8's second set (`1×3 (×3), 3×3`) is the *easier* of that stage's two, and its table cuts the bomb rate rather than raising it. At 7,200 trials per set: bombs off, it comes in **8.1% cheaper** than stage 8's other set; with its real bomb rate applied that gap closes to **0.8%**. Same compensation, same sizing, opposite sign, which is a fourth data point for the mapping. It went unkeyed while the set was unrecorded and is keyed as `"8:1"` now that a screenshot filled the set in.

That leaves **one** unverified input: bomb targeting is assumed uniform over unopened cells. That is why the toggle still defaults off.
...
[`tests/app.test.js`](../tests/app.test.js) loads `index.html` in jsdom and drives the real DOM (`node --test`, i.e. `npm test`). It covers boot, digging, the popover regression, preset loading (grid, pickaxes, and the stage's first set), the **set picker** (shown only on multi-set stages, labelled by dimensions, hidden again on a manual edit) and the partial-stage hint, a **data check** that every one of the 37 sets has at least one valid layout on its board and that no stage lists the same set twice, the no-treasures status line, the **patch notice** (opens once per browser, remembered by version, reopened from the setup panel, and kept shut when `localStorage` is unavailable), the custom-switch, the best-tile highlight, the dug/buried split and toggle, the empty-board path, the estimator (including the regression that it counts every treasure tile, not one hit per treasure), the **DP toggle** (exact `(DP)` engine on a dense stage where DFS bails, and the Monte-Carlo fallback when it's off), **persistence** (a board round-tripping through a refresh, restored treasure identity, a board saved on a stage's second set coming back on that set, the stale-preset fallback to custom, corrupt saves ignored, and New game not resurrecting the old board), **i18n** (the picker's per-locale links, a prerendered page booting in its language, region-aware auto-detection on the root, and that treasure names never surface), and the **per-locale pages** (reciprocal `hreflang`, self-canonicals, the pin outranking a stored preference, `../` asset paths, and the sitemap).
