...
- **Stage presets** — pick any of the 24 stages to auto-fill the grid size, the pickaxes-per-tile, and the treasures. The game picks each stage's treasures at random from several sets of similar difficulty, so a preset is a *list* of sets: 16 stages have only one and just load it, and the other 8 offer a picker so you can choose the set the game dealt you (they are listed by dimension, e.g. `1×2 (×2), 2×2 (×2), 2×4`). Two sets are still unrecorded, on stages 8 and 9; those stages say so, and you can enter the treasures by hand as before.
...
1. Pick a stage from the **Preset** dropdown (or set the grid size yourself). If the stage offers a **Treasure set** picker, choose the one matching what the game shows; on a custom board, add the treasures by dimension yourself.
...
- **Pick-cost estimate:** finding a treasure isn't collecting it — every treasure tile has to be dug out. So picks-to-finish = the empty tiles you waste while hunting for unlocated treasures (estimated by sampling many real layouts and simulating greedy play) **plus** every treasure tile still to dig (unlocated treasure areas + buried tiles of located ones). By default it ignores bombs, which only make the real cost lower. Turn on **Account for bombs** to simulate them instead: a dig on an empty tile can set off a bomb that opens extra tiles (and collects treasure) for free, which lowers the estimate by roughly 10 to 15%. It's off by default because it rests on a couple of in-game assumptions that aren't fully pinned down yet.
